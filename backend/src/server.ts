...
import dns from 'dns';
import { ProjectModel, ExpertiseModel, BlogModel, MessageModel, ProfileModel } from './models';
...
async function seedDatabaseIfEmpty() {
  try {
    ...
    const expCount = await ExpertiseModel.countDocuments();
    if (expCount === 0) {
      await ExpertiseModel.insertMany(initialExpertise);
...
app.get('/api/health', (req: Request, res: Response) => {
  res.json({
    ...
    database: mongoose.connection.readyState === 1 ? 'Connected (MongoDB Atlas)' : 'Disconnected',
...
app.get('/api/projects/:idOrSlug', async (req: Request, res: Response) => {
  try {
...
app.post('/api/projects', async (req: Request, res: Response) => {
  try {
    ...
    const newProjectData = {
      ...
      techStack: Array.isArray(req.body.techStack) ? req.body.techStack : (req.body.techStack ? req.body.techStack.split(',').map((s: string) => s.trim()).filter(Boolean) : ['React.js', 'Express.js']),
      keywords: Array.isArray(req.body.keywords) ? req.body.keywords : (req.body.keywords ? req.body.keywords.split(',').map((s: string) => s.trim()).filter(Boolean) : ['Web App']),
      highlights: Array.isArray(req.body.highlights) ? req.body.highlights : (req.body.highlights ? req.body.highlights.split('\n').map((s: string) => s.trim()).filter(Boolean) : [description]),
...
app.put('/api/projects/:id', async (req: Request, res: Response) => {
  try {
    ...
    const updated = await ProjectModel.findOneAndUpdate(query, req.body, { new: true });
    if (!updated) {
      return res.status(404).json({ success: false, message: 'Project not found.' });
    }
...
app.get('/api/expertise', async (req: Request, res: Response) => {
  try {
    const items = await ExpertiseModel.find().sort({ createdAt: -1 });
...
app.get('/api/expertise/:slug', async (req: Request, res: Response) => {
  try {
    const { slug } = req.params;
    let item = await ExpertiseModel.findOne({ slug });
    if (!item && mongoose.Types.ObjectId.isValid(slug)) {
      item = await ExpertiseModel.findById(slug);
    }
    if (!item) {
      // Perform normalized slug fallback search
      const normQuery = slug.toLowerCase().replace(/[^a-z0-9]/g, '');
      const allItems = await ExpertiseModel.find();
...
app.post('/api/expertise', async (req: Request, res: Response) => {
  try {
    ...
    const created = await ExpertiseModel.create(newExpertiseData);
...
app.put('/api/expertise/:id', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    let query: any = { _id: id };
    if (!mongoose.Types.ObjectId.isValid(id)) {
    ...
    const updated = await ExpertiseModel.findOneAndUpdate(query, req.body, { new: true });
...
app.delete('/api/expertise/:id', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    let query: any = { _id: id };
    if (!mongoose.Types.ObjectId.isValid(id)) {
    ...
    await ExpertiseModel.findOneAndDelete(query);
...
app.post('/api/blogs', async (req: Request, res: Response) => {
  try {
    ...
    const newBlogData = {
      ...
      tags: Array.isArray(req.body.tags) ? req.body.tags : (req.body.tags ? req.body.tags.split(',').map((t: string) => t.trim()).filter(Boolean) : ['Full Stack']),
      featured: req.body.featured ?? true
    };
...
app.put('/api/blogs/:id', async (req: Request, res: Response) => {
  try {
    ...
    const updated = await BlogModel.findOneAndUpdate(query, req.body, { new: true });
...
app.delete('/api/blogs/:id', async (req: Request, res: Response) => {
  try {
    ...
    await BlogModel.findOneAndDelete(query);
...
app.post('/api/contact', async (req: Request, res: Response) => {
  try {
    ...
    const newMessage = await MessageModel.create({ name, email, message, date: new Date().toISOString() });
...
app.delete('/api/messages/:id', async (req: Request, res: Response) => {
  try {
    ...
    await MessageModel.findOneAndDelete(query);
